...
foreach($activities as $activity) {    
   ...
   $style = '--layout-row: ' . $activity->type_rownumber + 2 . ';';
   $printing_style =  '--printing-row: ' . $activity->printing_rownumber + 2 . '; --printing-column: ' . $activity->printing_columnnumber . ';';
   if($filter != 0) {
   if($activity->activity_column == 'FULLDAY' and $activity->type_rownumber >= $group_index) {
       $style = '--layout-row: ' . $activity->type_rownumber + (int) $group_index . ';';
   } elseif($activity->activity_column == 'FULLDAY') {
       $style = 'display: none;';
   }
   }
   ?>
    <section style="<? echo $style; echo $printing_style; ?>" class="activity activity-<?php echo strtolower($activity->activity_column); ?>">
